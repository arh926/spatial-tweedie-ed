...
setwd("/Users/aritrahalder/Dropbox/PhD Research/paper 1/code/")
rm(list=ls())
require(tweedie)
require(zipcode)
data(zipcode)
...
splits <- seq(min(ct_zip$latitude), max(ct_zip$latitude), length.out = 5)
speff <- sapply(ct_zip$latitude, function(x){
  if(x<=splits[2]) return(vec[1])
  else if(x<=splits[3] & x>splits[2]) return(vec[2])
  else if(x<=splits[4] & x>splits[3]) return(vec[3])
  else if(x>=splits[4]) return(vec[4])
})

col <- sapply(speff, function(x){
  if(x==vec[1]) return("darkred")
  else if(x==vec[2]) return("orange")
  else if(x==vec[3]) return("green")
  if(x==vec[4]) return("blue")
})
...
for(i in 1:length(N)){
  for(j in 1:length(psiL)){
      ...
      obj <- list(data=sim_data,
                  ...
                  X=x_mat,
                  Z=z_mat)
      save(obj, file=paste("/Users/aritrahalder/Desktop/paper1_code/sim_data/blockwise_",i,j,".RData",sep=""))
  }
}
